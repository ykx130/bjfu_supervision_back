...
def update_database(info=None):
    ...
    cursor.execute("select * from bjlydx.v_kckbsj where COURSE_YEAR='{}' and COURSE_SEMESTER='{}' and ROWNUM <10".format(info['year'], info['semester']))
    cols = [d[0] for d in cursor.description]
    all_data = cursor.fetchall()
    dao.OriginLessons.delete_all()
    print("查到课程数量: ", len(all_data))
    for raw_data in all_data:
        data = dict(zip(cols, raw_data))
        print("插入 {}".format(raw_data))
        lesson_weekday= None
        lesson_time = None
...
if __name__ == '__main__':
    ...
    info = {'host': args.host, 'user': args.user, 'passwd': args.passwd, 'db': args.db, 'year':args.year, 'semester': args.semester}
